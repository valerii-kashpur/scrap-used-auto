...
if __name__ == "__main__":
    ...
    scheduler.add_job(
        run_spider,
        "cron",
        hour=int(scraping_time.split(":")[0]),
        minute=int(scraping_time.split(":")[1]),
    )
    ...
    scheduler.add_job(
        create_db_dump,
        "cron",
        hour=int(dump_time.split(":")[0]),
        minute=int(dump_time.split(":")[1]),
    )

    logger.info(
        f"Scheduler started. Scraping at {scraping_time}, Dumping at {dump_time}"
    )
    scheduler.start()
